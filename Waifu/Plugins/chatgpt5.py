...
async def fetch_data(api_url: str, query: str) -> str:
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(f"{api_url}?query={query}")
            response.raise_for_status()
            data = response.json()
            return data.get("data", "No response from the API.")
...
async def chatgpt5(_: Client, message: Message):
    ...
    api_response = await fetch_data(api_url_chat5, query)
...
async def chatgpt4(_: Client, message: Message):
    ...
    query = " ".join(message.command[1:])
    txt = await message.reply_text("**Wait patiently, requesting to API...**")
    api_response = await fetch_data(api_url_chat4, query)
    await txt.edit(api_response)
...
async def bard(_: Client, message: Message):
    ...
    query = " ".join(message.command[1:])
    txt = await message.reply_text("**Wait patiently, requesting to API...**")
    api_response = await fetch_data(api_url_bard, query)
    await txt.edit(api_response)
